...
<body>
	...
	<p>반복문은 일정한 코드를 원하는 수 만큼 반복하는 것을 목표로 합니다. 그 종류는 세가지를 사용할 수 있습니다. <code>for</code>, <code>while</code>, 그리고 <code>do-while</code>입니다.
	</p>

	<ol>
		<li>for 문
			<p><code>for</code> 문은 반복 횟수가 명확할 때 주로 사용됩니다. 초기화, 조건식, 증감식을 한 줄에 작성하여 반복을 제어합니다.</p> <pre class="line-numbers">
    <code class="language-java">for (초기화; 조건식; 증감식) {
        // 반복 실행할 코드
    }</code>
    </pre>
			<ul>
				<li>초기화는 <code>for</code>문 내부에서 활용할 변수를 선언함을 의미합니다. <code>for</code>문 내부에서만 생성되고 반복문이 종료되면 사라집니다.
				</li>
        ...
        <code class="language-java">for (int i = 0; i < 10; i++) {
			...
			<p>만약 <code>n</code>번의 반복을 원한다면 다음과 같이 <code>i</code>를 선언하고 0으로 초기화 한 후 변수가 <code>n</code>보다 작다면 <code>i</code>를 1씩 증감시키도록 합니다.
		</li>
		<li>while 문
	...
	<p>다음 코드는 언제 멈출까요? <code>while(ture)</code>는 영원히 멈추지 않습니다. 영원히 말이죠! 이러한 문제를 대처하기 위해 반복문은 제어 명령어를 사용할 수 있습니다. 반복문 내에서 <code>break</code>와 <code>continue</code> 키워드를 사용하여 제어 흐름을 변경할 수 있습니다.
	</p>
	<ul>
	...
	<p>헤당 코드는 i가 5일 때 반복문을 탈출할 수 있도록 제어합니다. <code>i</code>가 5와 같다면 <code>break</code> 명령어를 만나게 되고 <strong>배열 밖</strong>으로 진행하게 됩니다.
	</p>
	...
	<p><code>continue</code>를 활용한다면 원하는 조건에서 반복문 실행을 건너뛰고 다시 처음으로 돌아갈 수 있도록 할 수 있습니다. <code>break</code>와 차이점은 배열의 밖이 아니라 배열의 처음으로 돌아간다는 것입니다.</p>

	<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
		<h2>조건문</h2>
	</div>
	<p>자바 조건문은 특정 조건에 따라 코드의 실행 흐름을 제어하는 데 사용됩니다. 조건문을 사용하면 프로그램이 주어진 조건을 평가하고, 그 조건이 참일 때만 특정 코드를 실행하도록 할 수 있습니다. 자바에서 가장 기본적인 조건문은 <code>if</code>, <code>if-else</code>, <code>else if</code>, 그리고 <code>switch</code> 문입니다.
	</p>
    ...
    if (number > 0) {
		...
		<li><code>if-else</code> 문
			<p><code>if-else</code> 문은 조건이 참일 때는 <code>if</code> 블록을, 거짓일 때는 <code>else</code> 블록을 실행합니다.</p> <pre class="line-numbers">
    ...
    if (number > 0) {
		...
		<li><code>switch</code> 문
			<p><code>switch</code> 문은 하나의 변수 값에 따라 여러 경우(case)를 처리할 때 사용합니다. 각 <code>case</code>는 <code>break</code> 문으로 끝나며, <code>break</code> 문이 없으면 다음 <code>case</code>로 실행이 넘어갑니다.</p> <pre class="line-numbers">
